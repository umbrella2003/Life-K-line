...
import { scoreMbti, MBTI_QUESTIONS, TYPE_NICKNAMES, type MbtiAnswer } from "@/lib/mbti";
import { buildPersonalityPrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const typeName = TYPE_NICKNAMES[mbti.type] || "";

    let reading = order.result;
    if (!reading) {
      const messages = buildPersonalityPrompt(mbti, typeName);
      reading = await callDeepSeek(messages);
      await saveOrderResult(order.id, reading);
    }

    return NextResponse.json({ mbti, typeName, reading });
  } catch (err) {
