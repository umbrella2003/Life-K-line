...
import { calcZiweiChart, type ZiweiInput } from "@/lib/ziwei";
import { buildZiweiPrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const input = order.payload as ZiweiInput;
    const chart = calcZiweiChart(input);

    let reading = order.result;
    if (!reading) {
      const messages = buildZiweiPrompt(chart);
