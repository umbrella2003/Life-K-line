...
import type { BigFiveAnswer, BigFiveResult } from "@/lib/bigfive";

const STORAGE_KEY = "bigfive_test_v1";
...
export default function BigFivePage() {
  ...
  const answer = async (choice: BigFiveAnswer) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/bigfive/preview", {
        method: "POST",
  ...
  const onUnlocked = async (orderId: string) => {
    ...
    setUnlocking(true);
    try {
      const res = await fetch("/api/bigfive/analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <main className="min-h-screen bg-slate-950 px-4 sm:px-6 py-10 sm:py-14">
      <div className="max-w-2xl mx-auto">
        <PageHeader
          ...
          eyebrow="现代科学 · 心理学"
          title="大五人格模型测试"
          description={`基于国际学界广泛使用的大五人格模型(Big Five/OCEAN)的 ${total} 题测评,测一次即可,结果会保存在本机,无需重复作答。`}
        />
        ...
        {stage === "result" && saved && (
          <div className="space-y-6 animate-fade-up">
            <section className={cardClass("cyan")}>
              <p className="text-xs text-slate-500 mb-4">你的大五人格得分(0-100)</p>
